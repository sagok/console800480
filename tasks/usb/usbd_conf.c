...
//    HAL_GPIO_Init(GPIOA, &GPIO_InitStruct);
  ...
  else if(hpcd->Instance == USB_OTG_HS)
  {
    ...
    GPIO_InitStruct.Mode = GPIO_MODE_AF_PP;
    GPIO_InitStruct.Pull = GPIO_NOPULL;
    GPIO_InitStruct.Speed = GPIO_SPEED_HIGH;//GPIO_SPEED_HIGH;
...
void HAL_PCD_SuspendCallback(PCD_HandleTypeDef *hpcd)
{
  ...
  { 	
    ...
    /*wait tiemout of 6 ULPI PHY clock ~= 18 cpu clocks @120 MHz*/
    for (i=0; i<18; i++)
    {
      __NOP();
    }
...
{    
  ...
  /* Be aware that enabling DMA mode will result in data being sent only by
     ...
     unless required. */
  hpcd.Init.dma_enable = 0;
  hpcd.Init.low_power_enable = 0;	//0
  hpcd.Init.phy_itface = PCD_PHY_ULPI;//PCD_PHY_ULPI;//PCD_PHY_EMBEDDED
  hpcd.Init.Sof_enable = 0;
  hpcd.Init.speed = PCD_SPEED_HIGH;//PCD_SPEED_HIGH;
  hpcd.Init.vbus_sensing_enable = 0;
  /* Link The driver to the stack */
  hpcd.pData = pdev;
  pdev->pData = &hpcd;
